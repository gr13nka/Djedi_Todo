...
- **Time-boxing** (replaces the old date-scoped `todayTasks` table): every `ProjectTask` carries `timeBox` (`'today' | 'week' | 'later' | 'someday'`, in descending order of commitment), an optional `scheduledDate` pin, and a cross-project `timeBoxOrder`. `hooks/useTaskBox.ts` is the live-query + move/reorder/toggle surface for one box; `/today` (`TodayPage`) is `useTaskBox('today')`, and `/tasks` (`TaskSelectionView`) has box tabs (`today`/`week`/`later`/`someday`/`all`, defaulting to **week**). `hooks/useTaskRollover.ts` runs `db/rollover.ts`'s `computeRollover()` on mount and on tab-visibility-regain: it demotes everything left in `'today'` (incomplete → `week`, completed → `later`) and promotes any task whose `scheduledDate` is due, all inside one Dexie transaction that also stamps `settings.lastRolloverDate` (the idempotency guard).
  - `'someday'` is the uncommitted backlog and the **default for every new task** — `db/taskOps.ts`'s `createProjectTask` takes `timeBox` as a defaulted parameter, so the in-project add form and Inbox sorting both inherit it without passing anything. The one caller that does pass a box is `/tasks`' quick-add, which hands over the active box tab so a task added under the `week` tab doesn't vanish from the view that created it.
  - Rollover only ever *empties* `'today'`; nothing decays out of `week`/`later`/`someday` on its own. That is what keeps `'someday'` a backlog rather than a staging area, and it is why adding a fourth box needed no rollover change.
  - Adding a fifth box value would be caught by the compiler at the three exhaustive `Record<TimeBox, …>` sites (`SelectableTaskRow.tsx`, `TaskSelectionView.tsx`, `db/index.ts`) but **not** at `MOVE_TARGETS`/`BOX_TABS` or the vault serializer, which accepts any string (`t.timeBox || 'later'`) and so round-trips a new value with no changes.
...
| `/tasks` | `TaskSelectionPage` | Wide. All tasks by project/folder; box tabs (today/week/later/someday/all), default **week** |
...
13 prebuilt themes (data objects in `theme/themes.ts`'s `PREBUILT_THEMES` — `light`, the two `wax-*`, gruvbox/everforest/catppuccin/solarized light+dark pairs, `nord`, `dracula`), plus `custom` (user-defined, colors in `settingsStore.customThemeColors`).
...
- **Input chrome and depth**: the container carries the chrome, the field is naked. `ui/TaskTextArea.tsx` and `ui/InlineTextEdit.tsx` are deliberately unstyled (`bg-transparent outline-none`); the wrapper supplies the surface. `components/inbox/InboxView.tsx`'s capture bar is the canonical add-bar: `rounded-2xl bg-bg-card p-2.5` + `boxShadow: NEU.pressed`, with a `rounded-xl text-accent` submit carrying `NEU.raisedSm`. Three rules the themes enforce: depth comes only from `utils/shadows.ts`'s `NEU.*` vars via inline `style`, **never** a Tailwind `shadow-*` (the vars are transparent in light themes, real shadows in dark, hairline rings in the wax themes, so a hardcoded shadow is wrong in at least two); radii stay `rounded-2xl`/`rounded-xl`, the only two `html.wax-pencil` rewrites; and a foreground on `bg-accent` is `text-accent-fg`, never `text-white`, which is unreadable on the light-accent themes. A glowing focus ring is not a convention here — rings mean drop target or selection, and keyboard-only focus is `focus-visible:ring-2 ring-accent`. A bare `border-dashed` means drop zone (`ProjectGrid.tsx`), not "input".
...
- **Time-boxing**: today/week/later/someday boxes on every task (`ProjectTask.timeBox`), manual promote/demote, optional `scheduledDate` pin, automatic daily rollover. New tasks start in `someday`. Replaces the old date-scoped Today List.
